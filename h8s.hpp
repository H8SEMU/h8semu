...
class H8SMemoryDevice : public H8SDevice
{
public:
	H8SMemoryDevice(uint8_t* _mem) : memory(_mem) {}
...
public:
	uint8_t memory[1<<24] {};
	h8reg regs[8] {};
	uint8_t *pc {0};		// program counter. pc is ALWAYS a multiple of 2. All instructions are 2bytes
	uint8_t ccr {128};
	uint8_t exr {0};
	H8SDevice* maps[1<<24] {};
	unsigned long long  cycles {0};
	unsigned long long pending_irqs {0};
	H8SMemoryDevice memoryDevice;
	...
	h8state() : memoryDevice(memory)
	{
		memmap(&memoryDevice, 0, sizeof(maps) / sizeof(maps[0]));
	}
	
	int se8(int in) {in&=0xff;return (in&0x80)?(in|0xFFFFFF00):in;}
	int se16(int in) {in&=0xffff;return (in&0x8000)?(in|0xFFFF0000):in;}
	
	int8_t &reg8(int rd) 	{return (rd&8)?regs[rd&7].rl:regs[rd&7].rh;}
	int16_t& reg16(int rd) {return (rd&8)?regs[rd&7].e:regs[rd&7].r;}
	int32_t& reg32(int rd)	{return regs[rd&7].er;}
	uint32_t getSP() const	{return regs[7].er;}
	void setSP(uint32_t s)	{regs[7].er=s;}
	...
	uint32_t addr(uint8_t *p) {return (uint32_t)(p-memory);}
	...
	void loadmem(const uint8_t* data,uint32_t size,uint32_t address)
	{
		memcpy(memory + address,data,size);
	}
	
	void readMemory(uint8_t* to, int from, int len)
	{
	...
	uint8_t *makepc(int address) {return address+memory;}
	int pcoff(uint8_t* pc) const {return (int)(pc - memory);}
	
	// memory accesses should use these
	int32_t read32(uint8_t *from) {return read32((int)(from-memory));}
	int16_t read16(uint8_t *from) {return read16((int)(from-memory));}
	int8_t read8(uint8_t *from) {return read8((int)(from-memory));}
	
	void write32(int32_t val,int to) {write16(val>>16,to);write16(val&0xffff,to+2);}
	uint32_t read32(int from) {uint32_t s=read16(from)&0xffff;s=(s<<16)|(read16(from+2)&0xffff);return s;}
	void write16(int16_t val,int to) {write8(val>>8,to);write8(val&255,to+1);}
	uint16_t read16(int from) {uint16_t s=read8(from)&255;s=(s<<8)|(read8(from+1)&255);return s;}
	
	void write8(int8_t byte,int to) {
		to&=0xffffff;
		clockMem(to, lastwrite);
		maps[to]->write(to, byte);
	}
	int8_t read8(int from) {
		from&=0xffffff;
		clockMem(from, lastread);
		return maps[from]->read(from);
	}
	
	void pushL(uint32_t val)	{uint32_t sp=getSP()-4;write32(val,sp);setSP(sp);}
	uint32_t popL() 			{uint32_t sp=getSP();setSP(sp+4);return read32(sp);}
	void pushW(uint16_t val)	{uint16_t sp=getSP()-2;write16(val,sp);setSP(sp);}
	uint16_t popW() 			{uint16_t sp=getSP();setSP(sp+2);return read16(sp);}
	
	void pushPC(uint8_t *pc,bool andccr=false)
	{
		uint32_t p=(uint32_t)((pc-memory)&0xffffff);
		if (andccr) pushL(p | (ccr <<24)); else pushL(p);
	}
	uint8_t *popPC(bool andccr=false) {
		uint32_t p = popL();
		if (andccr) ccr = p>>24;
		return (p & 0xffffff) + memory;
	}
	
	uint8_t *loadPCFrom(uint32_t addr) {return (read32(addr)&0xFFFFFF)+memory;}
...
public:
	...
	uint8_t *handle_instr(uint8_t *_pc)
	{
		pc = _pc;
...
protected:
	uint8_t *handle_instr_0(uint8_t op,uint8_t *pc)
	{
		switch (op&15)
		{
	...
	uint8_t *handle_instr_1(uint8_t op,uint8_t *pc)
	{
		switch (op&15)
		{
	...
	uint8_t *handle_instr_5(uint8_t op,uint8_t *pc)
	{
		switch (op&15)
		{
	...
	uint8_t *handle_instr_6(uint8_t op,uint8_t *pc)
	{
		switch (op&15)
		{
	...
	uint8_t *handle_instr_7(uint8_t op,uint8_t *pc)
	{
		switch (op&15)
		{
	...
	uint8_t *handle_instr_01(uint8_t op,uint8_t *pc)
	{
		int b=read8(pc);pc++;
		switch ((b>>4)&15)
		{
	...
	uint8_t *handle_instr_0a(uint8_t op,uint8_t *pc)
	{
		int b=read8(pc);pc++;
		switch ((b>>4)&15)
		{
	...
	uint8_t *handle_instr_0b(uint8_t op,uint8_t *pc)
	{
		int b=read8(pc);pc++;
		switch ((b>>4)&15)
		{
	...
	uint8_t *handle_instr_0f(uint8_t op,uint8_t *pc)
	{
		int b=read8(pc);pc++;
		switch ((b>>4)&15)
		{
	...
	uint8_t *handle_instr_10(uint8_t op,uint8_t *pc)
	{
		int b=read8(pc);pc++;
	...
	uint8_t *handle_instr_11(uint8_t op,uint8_t *pc)
	{
		int b=read8(pc);pc++;
		int top=(b>>4)&7; if (top==2 || top==6) return fail(pc-2);
	...
	uint8_t *handle_instr_12(uint8_t op,uint8_t *pc)
	{
		int b=read8(pc);pc++;
		int top=(b>>4)&7; if (top==2 || top==6) return fail(pc-2);
	...
	uint8_t *handle_instr_13(uint8_t op,uint8_t *pc)
	{
		int b=read8(pc);pc++;
		int top=(b>>4)&7; if (top==2 || top==6) return fail(pc-2);
	...
	uint8_t *handle_instr_17(uint8_t op,uint8_t *pc)
	{
		int b=read8(pc);pc++;
		switch ((b>>4)&15)
		{
	...
	uint8_t *handle_instr_1a(uint8_t op,uint8_t *pc)
	{
		int b=read8(pc);pc++;
	...
	uint8_t *handle_instr_1b(uint8_t op,uint8_t *pc)
	{
		int b=read8(pc);pc++;
		switch ((b>>4)&15)
		{
		...
		case 12:return handle_decwi(op, b, pc, 2);
		case 15:return handle_decli(op, b, pc, 2);
		default:return fail(pc-2);
		}
	}
	uint8_t *handle_instr_1f(uint8_t op,uint8_t *pc)
	{
		int b=read8(pc);pc++;
	...
	uint8_t *handle_instr_6a(uint8_t op,uint8_t *pc)
	{
		int b=read8(pc);pc++;
		switch ((b>>4)&15)
		{
	...
	uint8_t *handle_instr_79(uint8_t op,uint8_t *pc)
	{
		int b=read8(pc);pc++;
		switch ((b>>4)&15)
		{
	...
	uint8_t *handle_instr_7a(uint8_t op,uint8_t *pc)
	{
		int b=read8(pc);pc++;
		switch ((b>>4)&15)
		{
	...
	uint8_t *handle_instr_01c(uint8_t op, uint8_t b, uint8_t *pc)
	{
	...
	uint8_t *handle_instr_01d(uint8_t op, uint8_t b, uint8_t *pc)
	{
		if (b&15) return fail(pc-2);
		int c=read8(pc);pc++;
		int d=read8(pc);pc++;
	...
	uint8_t *handle_instr_01f(uint8_t op, uint8_t b, uint8_t *pc)
	{
		if (b&15) return fail(pc-2);
		int c=read8(pc);pc++;
		int d=read8(pc);pc++;
	...
	//////// MOV.B @immediate,Reg
	uint8_t *handle_movba(uint8_t op,uint8_t *pc)
	{
	...
	uint8_t *handle_movbaa(uint8_t op,uint8_t b,uint8_t *pc)
	{
		uint8_t *startpc=pc-2;int rd=b&15,regsrc=(b&0x80)?1:0,size=(b&0x20)?1:0;
	...
	uint8_t *handle_movbae(uint8_t op,uint8_t *pc)
	{
	...
	uint8_t *handle_movwae(uint8_t op,uint8_t *pc)
	{
		int r=read8(pc);pc++;int er=(r>>4)&7,d=(r>>7)&1;r&=15;
		if (disassemble)
		{
	...
	uint8_t *handle_movwaa(uint8_t op,uint8_t *pc)
	{
		uint8_t *ipc=pc-1;int r=read8(pc);pc++;int sub=(r>>4)&15;r&=15;int d=sub&8;
	...
	uint8_t *handle_movbaep(uint8_t op,uint8_t *pc)
	{
	...
	uint8_t *handle_movwaep(uint8_t op,uint8_t *pc)
	{
		int r=read8(pc);pc++;int e=(r>>4)&7,d=(r>>7)&1;r&=15;
		if (disassemble)
		{
	...
	uint8_t *handle_movbaeo(uint8_t op,uint8_t *pc)
	{
	...
	uint8_t *handle_movwaeo(uint8_t op,uint8_t *pc)
	{
		int r=read8(pc);pc++;int e=(r>>4)&7,d=(r>>7)&1;r&=15;int offset=read16(pc);pc+=2;
		if (disassemble)
		{
	...
	uint8_t *handle_movbwaed(uint8_t op,uint8_t *pc)
	{
	...
	uint8_t* handle_movl(uint8_t op,uint8_t b,uint8_t *pc)
	{
		uint8_t *pcstart=pc-2;
		if (b) return fail(pcstart);
		uint8_t c=read8(pc);pc++;
	...
	uint8_t *handle_movrl(uint8_t op,uint8_t b,uint8_t *pc)
	{
		if (b&0x8) return fail(pc-2);
		int rd=(b&7),rs=(b>>4)&7;
		if (disassemble)	dasm(pc-2,"MOV.L","%s,%s",getRegName32(rs),getRegName32(rd));
		if (execute) {int32_t v=reg32(rs);ccrflags_val(v);reg32(rd)=v;}
		return pc;
	}
	
	//////// LDM/STM
	uint8_t* handle_ldmstm(uint8_t op,uint8_t b,uint8_t *pc)
	{
	...
	uint8_t* handle_ldcstc(uint8_t op,uint8_t b,uint8_t *pc)
	{
		uint8_t *pcstart=pc-2;
		if ((b&0xfe)!=0x40) return fail(pcstart);
		int isexr=(b&1);	// if not exr then ccr
		uint8_t &cr=isexr?exr:ccr;
		int c=read8(pc);pc++;
		int d=read8(pc);pc++;
		if (isexr)
		{
	...
	uint8_t *handle_tas(uint8_t op,uint8_t b,uint8_t *pc)
	{
		...
		if (execute)	{uint8_t b=read8(reg32(r));ccrflags_val(b);write8(b|0x80,reg32(r));}
		return pc;
	}
	
	//////// REGISTER OPS
	uint8_t *handle_movr(uint8_t op,uint8_t *pc)
	{
		int regs=read8(pc);pc++;	int rs=(regs>>4)&15,rd=(regs)&15;
		if (disassemble)	dasm(pc-2,"MOV.B","%s,%s",getRegName8(rs),getRegName8(rd));
		if (execute) {int8_t &src=reg8(rs), &dst=reg8(rd);	ccrflags_val(src);	dst=src; }
		return pc;
	}
	uint8_t *handle_movrw(uint8_t op,uint8_t *pc)
	{
		int regs=read8(pc);pc++;	int rs=(regs>>4)&15,rd=(regs)&15;
		if (disassemble)	dasm(pc-2,"MOV.W","%s,%s",getRegName16(rs),getRegName16(rd));
		if (execute) {int16_t &src=reg16(rs),&dst=reg16(rd);	ccrflags_val(src);	dst=src; }
		return pc;
	}

	uint8_t *handle_orr(uint8_t op,uint8_t *pc)
	{
		int regs=read8(pc);pc++;	int rs=(regs>>4)&15,rd=(regs)&15;
		if (disassemble)	dasm(pc-2,"OR.B","%s,%s",getRegName8(rs),getRegName8(rd));
		if (execute) {int8_t &src=reg8(rs),&dst=reg8(rd);	dst|=src;	ccrflags_val(dst); }
		return pc;
	}

	uint8_t *handle_orrw(uint8_t op,uint8_t *pc)
	{
		int regs=read8(pc);pc++;	int rs=(regs>>4)&15,rd=(regs)&15;
		if (disassemble)	dasm(pc-2,"OR.W","%s,%s",getRegName16(rs),getRegName16(rd));
		if (execute) {int16_t &src=reg16(rs),&dst=reg16(rd);	dst|=src;	ccrflags_val(dst); }
		return pc;
	}
	uint8_t *handle_orrl(uint8_t d,uint8_t *pc)
	{
		int rs=(d>>4)&15,rd=(d)&15;
		if (disassemble)	dasm(pc-4,"OR.L","%s,%s",getRegName32(rs),getRegName32(rd));
		if (execute) {int32_t &src=reg32(rs),&dst=reg32(rd);	dst|=src;	ccrflags_val(dst); }
		return pc;
	}

	uint8_t *handle_xorr(uint8_t op,uint8_t *pc)
	{
		int regs=read8(pc);pc++;	int rs=(regs>>4)&15,rd=(regs)&15;
	...
	uint8_t *handle_xorrw(uint8_t op,uint8_t *pc)
	{
		int regs=read8(pc);pc++;	int rs=(regs>>4)&15,rd=(regs)&15;
	...
	uint8_t *handle_xorrl(uint8_t d,uint8_t *pc)
	{
		int rs=(d>>4)&15,rd=(d)&15;
	...
	uint8_t *handle_andr(uint8_t op,uint8_t *pc)
	{
		int regs=read8(pc);pc++;	int rs=(regs>>4)&15,rd=(regs)&15;
	...
	uint8_t *handle_andrw(uint8_t op,uint8_t *pc)
	{
		int regs=read8(pc);pc++;	int rs=(regs>>4)&15,rd=(regs)&15;
	...
	uint8_t *handle_andrl(uint8_t d,uint8_t *pc)
	{
		int rs=(d>>4)&15,rd=(d)&15;
	...
	uint8_t *handle_incb(uint8_t op,uint8_t b,uint8_t *pc)
	{
		int r=b&0xf;
	...
	uint8_t *handle_decb(uint8_t op,uint8_t b,uint8_t *pc)
	{
		int r=b&0xf;
	...
	uint8_t *handle_incwi(uint8_t op,uint8_t b,uint8_t *pc,int imm)
	{
		int r=b&0xf;
	...
	uint8_t *handle_decwi(uint8_t op,uint8_t b,uint8_t *pc,int imm)
	{
		int r=b&0xf;
	...
	uint8_t *handle_incli(uint8_t op,uint8_t b,uint8_t *pc,int imm)
	{
		if (b&0x8) return fail(pc-2);
		int r=b&0xf;
	...
	uint8_t *handle_decli(uint8_t op,uint8_t b,uint8_t *pc,int imm)
	{
		if (b&0x8) return fail(pc-2);
		int r=b&0xf;
	...
	uint8_t *handle_addr(uint8_t op,uint8_t *pc)
	{
		int regs=read8(pc);pc++;	int rs=(regs>>4)&15,rd=(regs)&15;
	...
	uint8_t *handle_addxr(uint8_t op,uint8_t *pc)
	{
		int regs=read8(pc);pc++;	int rs=(regs>>4)&15,rd=(regs)&15;
	...
	uint8_t *handle_addrw(uint8_t op,uint8_t *pc)
	{
		int regs=read8(pc);pc++;	int rs=(regs>>4)&15,rd=(regs)&15;
	...
	uint8_t *handle_addsi(uint8_t op,uint8_t b,uint8_t *pc,int imm)
	{
		if (b&0x8) return fail(pc-2);
		int rd=(b&7);
	...
	uint8_t *handle_subsi(uint8_t op,uint8_t b,uint8_t *pc,int imm)
	{
		if (b&0x8) return fail(pc-2);
		int rd=(b&7);
	...
	uint8_t *handle_addl(uint8_t op,uint8_t b,uint8_t *pc)
	{
		if (b&0x8) return fail(pc-2);
		int rs=(b>>4)&7,rd=(b&7);
	...
	uint8_t *handle_subr(uint8_t op,uint8_t *pc)
	{
		int regs=read8(pc);pc++;	int rs=(regs>>4)&15,rd=(regs)&15;
	...
	uint8_t *handle_subxr(uint8_t op,uint8_t *pc)
	{
		int regs=read8(pc);pc++;	int rs=(regs>>4)&15,rd=(regs)&15;
	...
	uint8_t *handle_subrw(uint8_t op,uint8_t *pc)
	{
		int regs=read8(pc);pc++;	int rs=(regs>>4)&15,rd=(regs)&15;
	...
	uint8_t *handle_subl(uint8_t op,uint8_t b,uint8_t *pc)
	{
	...
	uint8_t *handle_cmpl(uint8_t op,uint8_t b,uint8_t *pc)
	{
		if (b&8) return fail(pc-2);
		int rd=b&7,rs=(b>>4)&7;
	...
	uint8_t *handle_cmpr(uint8_t op,uint8_t *pc)
	{
		int regs=read8(pc);pc++;	int rs=(regs>>4)&15,rd=(regs)&15;
	...
	uint8_t *handle_cmprw(uint8_t op,uint8_t *pc)
	{
		int regs=read8(pc);pc++;	int rs=(regs>>4)&15,rd=(regs)&15;
	...
	uint8_t *handle_mulxu(uint8_t op,uint8_t *pc)
	{
		int regs=read8(pc);pc++;	int rs=(regs>>4)&15,rd=(regs)&15;
		if (disassemble)	dasm(pc-2,"MULXU.B","%s,%s",getRegName8(rs),getRegName16(rd));
		if (execute) {int a=(reg8(rs)&255),b=(reg16(rd)&255);int16_t &d=reg16(rd);d=a*b; clockI(12);}
		return pc;
	}
	uint8_t *handle_mulxs(uint8_t d,uint8_t *pc)
	{
		int rs=(d>>4)&15,rd=(d)&15;
		if (disassemble)	dasm(pc-4,"MULXS.B","%s,%s",getRegName8(rs),getRegName16(rd));
		if (execute) {int a=reg8(rs),b=se8(reg16(rd));int16_t &d=reg16(rd);d=a*b;ccrflags_set(ccr_n,d<0);ccrflags_set(ccr_z,!d); clockI(12);}
		return pc;
	}
	uint8_t *handle_divxu(uint8_t op,uint8_t *pc)
	{
		int regs=read8(pc);pc++;	int rs=(regs>>4)&15,rd=(regs)&15;
	...
	uint8_t *handle_divxs(uint8_t d,uint8_t *pc)
	{
		int rs=(d>>4)&15,rd=(d)&15;
		if (rd&8) printf("UNDEFINED BEHAVIOUR. Attempting to divide E register\n");
	...
	uint8_t *handle_mulxuw(uint8_t op,uint8_t *pc)
	{
		int regs=read8(pc);pc++;	int rs=(regs>>4)&15,rd=(regs)&15;
		if (rd&8) return fail(pc-2);
		if (disassemble)	dasm(pc-2,"MULXU.W","%s,%s",getRegName16(rs),getRegName32(rd));
		if (execute) {uint32_t a=(reg16(rs)&65535),b=(reg32(rd)&65535);reg32(rd)=a*b; clockI(20);}
		return pc;
	}
	uint8_t *handle_mulxsw(uint8_t d,uint8_t *pc)
	{
		int rs=(d>>4)&15,rd=(d)&15;
		if (rd&8) return fail(pc-4);
		if (disassemble)	dasm(pc-4,"MULXS.W","%s,%s",getRegName16(rs),getRegName32(rd));
		if (execute) {int a=reg16(rs),b=se16(reg32(rd));int32_t &d=reg32(rd);d=a*b;ccrflags_set(ccr_n,d<0);ccrflags_set(ccr_z,!d); clockI(20);}
		return pc;
	}
	uint8_t *handle_divxuw(uint8_t op,uint8_t *pc)
	{
		int regs=read8(pc);pc++;	int rs=(regs>>4)&15,rd=(regs)&15;
		if (rd&8) return fail(pc-2);
		...
		if (execute)
		{
			uint32_t a=reg16(rs)&65535,b=reg32(rd), c = 0;
	...
	uint8_t *handle_divxsw(uint8_t d,uint8_t *pc)
	{
		int rs=(d>>4)&15,rd=(d)&15;
		if (rd&8) return fail(pc-4);
		...
		{
			clockI(20);
			int32_t a=se16(reg16(rs)),b=reg32(rd), c = 1;
			ccrflags_set(ccr_z,!a);
			if (a!=0) c=b/a;
	...
	//////// Shifts
	uint8_t *handle_shal(uint8_t op, uint8_t b, uint8_t *pc)
	{
	...
	uint8_t *handle_shll(uint8_t op, uint8_t b, uint8_t *pc)
	{
		int shift=(b&64)?2:1, size=((b>>4)&3)+1, r=(b&15);	// size=1,2,4
		if (size==4 && (r&8)) return fail(pc-2);
		if (disassemble)
	...
	uint8_t *handle_shar(uint8_t op, uint8_t b, uint8_t *pc)
	{
		int shift=(b&64)?2:1, size=((b>>4)&3)+1, r=(b&15);	// size=1,2,4
		if (size==4 && (r&8)) return fail(pc-2);
		if (disassemble)
	...
	uint8_t *handle_shlr(uint8_t op, uint8_t b, uint8_t *pc)
	{
		int shift=(b&64)?2:1, size=((b>>4)&3)+1, r=(b&15);	// size=1,2,4
		if (size==4 && (r&8)) return fail(pc-2);
		if (disassemble)
	...
	uint8_t *handle_rotl(uint8_t op, uint8_t b, uint8_t *pc)
	{
		int shift=(b&64)?2:1, size=((b>>4)&3)+1, r=(b&15);	// size=1,2,4
		if (size==4 && (r&8)) return fail(pc-2);
		if (disassemble)
	...
	uint8_t *handle_rotxl(uint8_t op, uint8_t b, uint8_t *pc)
	{
		int shift=(b&64)?2:1, size=((b>>4)&3)+1, r=(b&15);	// size=1,2,4
		if (size==4 && (r&8)) return fail(pc-2);
		if (disassemble)
	...
	uint8_t *handle_rotr(uint8_t op, uint8_t b, uint8_t *pc)
	{
		int shift=(b&64)?2:1, size=((b>>4)&3)+1, r=(b&15);	// size=1,2,4
		if (size==4 && (r&8)) return fail(pc-2);
		if (disassemble)
	...
	uint8_t *handle_rotxr(uint8_t op, uint8_t b, uint8_t *pc)
	{
		int shift=(b&64)?2:1, size=((b>>4)&3)+1, r=(b&15);	// size=1,2,4
		if (size==4 && (r&8)) return fail(pc-2);
		if (disassemble)
	...
	//////// BITWISE OPS
	uint8_t *handle_bsetr(uint8_t op,uint8_t *pc)
	{
		int regs=read8(pc);pc++;	int rs=(regs>>4)&15,rd=(regs)&15;
	...
	uint8_t *handle_bseti(uint8_t op,uint8_t *pc)
	{
	...
	uint8_t *handle_bnotr(uint8_t op,uint8_t *pc)
	{
		int regs=read8(pc);pc++;	int rs=(regs>>4)&15,rd=(regs)&15;
	...
	uint8_t *handle_bnoti(uint8_t op,uint8_t *pc)
	{
		int regs=read8(pc);pc++;	int imm=(regs>>4)&15,rd=(regs)&15;
		if (imm&8) return fail(pc-2);
	...
	uint8_t *handle_bclrr(uint8_t op,uint8_t *pc)
	{
		int regs=read8(pc);pc++;	int rs=(regs>>4)&15,rd=(regs)&15;
	...
	uint8_t *handle_bclri(uint8_t op,uint8_t *pc)
	{
		int regs=read8(pc);pc++;	int imm=(regs>>4)&15,rd=(regs)&15;
		if (imm&8) return fail(pc-2);
	...
	uint8_t *handle_btstr(uint8_t op,uint8_t *pc)
	{
		int regs=read8(pc);pc++;	int rs=(regs>>4)&15,rd=(regs)&15;
	...
	uint8_t *handle_btsti(uint8_t op,uint8_t *pc)
	{
		int regs=read8(pc);pc++;	int imm=(regs>>4)&15,rd=(regs)&15;
		if (imm&8) return fail(pc-2);
	...
	uint8_t *handle_bsti(uint8_t op,uint8_t *pc)
	{
		int regs=read8(pc);pc++;	int imm=(regs>>4)&7,rd=(regs)&15,n=(regs>>7)&1;
		if (disassemble)	dasm(pc-2,n?"BIST":"BST","#%d,%s",imm,getRegName8(rd));
		if (execute)		{int8_t &r=reg8(rd);r&=~(1<<imm);r|=((ccr&1)^n)<<imm;}
		return pc;
	}
	
	uint8_t *handle_bor(uint8_t op,uint8_t *pc)
	{
		int regs=read8(pc);pc++;	int imm=(regs>>4)&7,rd=(regs)&15,n=(regs>>7)&1;
	...
	uint8_t *handle_bxor(uint8_t op,uint8_t *pc)
	{
		int regs=read8(pc);pc++;	int imm=(regs>>4)&7,rd=(regs)&15,n=(regs>>7)&1;
	...
	uint8_t *handle_band(uint8_t op,uint8_t *pc)
	{
		int regs=read8(pc);pc++;	int imm=(regs>>4)&7,rd=(regs)&15,n=(regs>>7)&1;
	...
	uint8_t *handle_bld(uint8_t op,uint8_t *pc)
	{
		int regs=read8(pc);pc++;	int imm=(regs>>4)&7,rd=(regs)&15,n=(regs>>7)&1;
	...
	uint8_t *handle_not(uint8_t op, uint8_t b, uint8_t *pc)
	{
	...
	uint8_t *handle_neg(uint8_t op, uint8_t b, uint8_t *pc)
	{
		int size=((b>>4)&3)+1, r=(b&15);	// size=1,2,4
		if (size==4 && (r&8)) return fail(pc-2);
		if (disassemble)
	...
	uint8_t *handle_extu(uint8_t op, uint8_t b, uint8_t *pc)
	{
	...
	uint8_t *handle_exts(uint8_t op, uint8_t b, uint8_t *pc)
	{
		int size=((b>>4)&3)+1, r=(b&15);	// size=2,4
		if (size==4 && (r&8)) return fail(pc-2);
		if (disassemble)
	...
	uint8_t *inner_bitstuff_aa(int c,int d,int aa,int type,uint8_t *startpc,uint8_t *pc)
	{
		...
		if (execute)
		{
			int bi=1<<imm,br=1<<reg8(rn),cc=ccr&1,byte;
			if (!type)
			{
				if (inv && c==0x73)	return fail(startpc);
				byte=read8(aa);
				switch (c)
				{
				case 0x63:	ccrflags_set(ccr_z,(byte&br)==0);			break;
			...
			else
			{
				...
				switch (c)
				{
				case 0x60:	byte|=br;	break;
				case 0x61:	byte^=br;	break;
				case 0x62:	byte&=~br;	break;
				case 0x67:	byte&=~br;byte|=(inv^cc)*br;		break;
	...
	uint8_t *handle_bitstuff_1(uint8_t op,uint8_t *pc)	// to get here, op==0x7c, 0x7d, 0x7e, 0x7f
	{
		int b=read8(pc);pc++;
		int c=read8(pc);pc++;
		int d=read8(pc);pc++;
	...
	uint8_t *handle_bitstuff_2(uint8_t op,uint8_t b,uint8_t *pc)	// op=6a, b=0x1? or 0x3?
	{
		uint8_t *startpc=pc-2;
		int aa=0;
	...
	//////// IMMEDIATE OPS
	uint8_t *handle_cmpi(uint8_t op,uint8_t *pc)
	{
	...
	uint8_t *handle_addi(uint8_t op,uint8_t *pc)
	{
		int rd=op&15,imm=read8(pc);pc++;
	...
	uint8_t *handle_addxi(uint8_t op,uint8_t *pc)
	{
		int rd=op&15,imm=read8(pc);pc++;
	...
	uint8_t *handle_subxi(uint8_t op,uint8_t *pc)
	{
		int rd=op&15,imm=read8(pc);pc++;
	...
	uint8_t *handle_ori(uint8_t op,uint8_t *pc)
	{
		int rd=op&15,imm=read8(pc);pc++;
		if (disassemble) dasm(pc-2,"OR.B","#0x%x,%s",imm,getRegName8(rd));
		if (execute) {int8_t &r=reg8(rd);	r|=imm;	ccrflags_val(r);}
		return pc;
	}
	uint8_t *handle_xori(uint8_t op,uint8_t *pc)
	{
		int rd=op&15,imm=read8(pc);pc++;
	...
	uint8_t *handle_andi(uint8_t op,uint8_t *pc)
	{
		int rd=op&15,imm=read8(pc);pc++;
	...
	uint8_t *handle_movbi(uint8_t op,uint8_t *pc)
	{
		int rd=op&15,imm=read8(pc);pc++;
	...
	uint8_t *handle_cmpwi(uint8_t op,uint8_t b,uint8_t *pc)
	{
	...
	uint8_t *handle_addwi(uint8_t op,uint8_t b,uint8_t *pc)
	{
		int rd=b&15,imm=read16(pc);pc+=2;
	...
	uint8_t *handle_subwi(uint8_t op,uint8_t b,uint8_t *pc)
	{
		int rd=b&15,imm=read16(pc);pc+=2;
	...
	uint8_t *handle_orwi(uint8_t op,uint8_t b,uint8_t *pc)
	{
		int rd=b&15,imm=read16(pc);pc+=2;
		if (disassemble) dasm(pc-4,"OR.W","#0x%x,%s",imm,getRegName16(rd));
		if (execute) {int16_t &r=reg16(rd);	r|=imm;	ccrflags_val(r);}
		return pc;
	}
	uint8_t *handle_xorwi(uint8_t op,uint8_t b,uint8_t *pc)
	{
		int rd=b&15,imm=read16(pc);pc+=2;
	...
	uint8_t *handle_andwi(uint8_t op,uint8_t b,uint8_t *pc)
	{
	...
	uint8_t *handle_movwi(uint8_t op,uint8_t b,uint8_t *pc)
	{
		int rd=b&15,imm=read16(pc)&0xffff;pc+=2;
	...
	uint8_t *handle_cmpli(uint8_t op,uint8_t b,uint8_t *pc)
	{
	...
	uint8_t *handle_addli(uint8_t op,uint8_t b,uint8_t *pc)
	{
		int rd=b&15,imm=read32(pc);pc+=4;if (rd&8) return fail(pc-6);
	...
	uint8_t *handle_subli(uint8_t op,uint8_t b,uint8_t *pc)
	{
		int rd=b&15,imm=read32(pc);pc+=4;if (rd&8) return fail(pc-6);
	...
	uint8_t *handle_orli(uint8_t op,uint8_t b,uint8_t *pc)
	{
		int rd=b&15,imm=read32(pc);pc+=4;if (rd&8) return fail(pc-6);
		if (disassemble) dasm(pc-6,"OR.L","#0x%x,%s",imm,getRegName32(rd));
		if (execute) {int32_t &r=reg32(rd);	r|=imm;	ccrflags_val(r); }
		return pc;
	}
	uint8_t *handle_xorli(uint8_t op,uint8_t b,uint8_t *pc)
	{
		int rd=b&15,imm=read32(pc);pc+=4;if (rd&8) return fail(pc-6);
	...
	uint8_t *handle_andli(uint8_t op,uint8_t b,uint8_t *pc)
	{
		int rd=b&15,imm=read32(pc);pc+=4;if (rd&8) return fail(pc-6);
	...
	uint8_t *handle_movli(uint8_t op,uint8_t b,uint8_t *pc)
	{
		int rd=b&15,imm=read32(pc);pc+=4;if (rd&8) return fail(pc-6);
	...
	uint8_t *handle_daa(uint8_t op,uint8_t b,uint8_t *pc)
	{
		int r=b&0xf;
	...
	uint8_t *handle_das(uint8_t op,uint8_t b,uint8_t *pc)
	{
		int r=b&0xf;
	...
	///////// CCR OPS
	uint8_t *handle_stcr(uint8_t op,uint8_t *pc)
	{
		int rd=read8(pc);pc++;
		if ((rd>>4)>1) return fail(pc-2);
		if (disassemble) dasm(pc-2,"STC.B","%s,%s",(rd&0xf0)?"EXR":"CCR",getRegName8(rd));
		if (execute) {int8_t &r=reg8(rd);	r=(rd&0xf0)?exr:ccr;}
		return pc;
	}
	uint8_t *handle_ldcr(uint8_t op,uint8_t *pc)
	{
		int rd=read8(pc);pc++;
		if ((rd>>4)>1) return fail(pc-2);
		if (disassemble) dasm(pc-2,"LDC.B","%s,%s",getRegName8(rd),(rd&0xf0)?"EXR":"CCR");
		if (execute) {const int8_t &r=reg8(rd);if (rd&0xf0) exr=r; else ccr=r; }
		return pc;
	}
	uint8_t *handle_orc(uint8_t op,uint8_t *pc)
	{
		int imm=read8(pc);pc++;
	...
	uint8_t *handle_xorc(uint8_t op,uint8_t *pc)
	{
		int imm=read8(pc);pc++;
	...
	uint8_t *handle_andc(uint8_t op,uint8_t *pc)
	{
		int imm=read8(pc);pc++;
	...
	uint8_t *handle_ldci(uint8_t op,uint8_t *pc)
	{
		int imm=read8(pc);pc++;
	...
	//////// BCC
	uint8_t *inner_bcc(uint8_t *startpc,uint8_t *pc,int cc,int imm)
	{
	...
	uint8_t *handle_bcc(uint8_t op,uint8_t *pc)
	{
		int cc=op&15,imm=read8(pc);pc++;
		return inner_bcc(pc-2,pc,cc,imm);
	}
	uint8_t *handle_bccw(uint8_t op,uint8_t *pc)
	{
		int b=read8(pc);pc++;
		if (b&0xf) return fail(pc-2);
	...
	uint8_t *handle_rts(uint8_t op,uint8_t *pc)
	{
	...
	uint8_t *handle_rte(uint8_t op,uint8_t *pc)
	{
		int test=read8(pc);pc++;
		if (test!=0x70) return fail(pc-2);
	...
	uint8_t *handle_bsr(uint8_t op,uint8_t *pc)
	{
		int imm=read8(pc);pc++;
	...
	uint8_t *handle_bsrw(uint8_t op,uint8_t *pc)
	{
		int imm=read8(pc);pc++;
	...
	uint8_t *handle_trapa(uint8_t op,uint8_t *pc)
	{
		int imm=read8(pc);pc++;
	...
	uint8_t *handle_jmpae(uint8_t op,uint8_t *pc)
	{
		int er=read8(pc);pc++;
	...
	uint8_t *handle_jmpimm(uint8_t op,uint8_t *pc)
	{
	...
	uint8_t *handle_jmpaa(uint8_t op,uint8_t *pc)
	{
	...
	uint8_t *handle_jsrae(uint8_t op,uint8_t *pc)
	{
		int er=read8(pc);pc++;
		if (er&0x8f) return fail(pc-2);
	...
	uint8_t *handle_jsrimm(uint8_t op,uint8_t *pc)
	{
		int addr=(read16(pc) << 8) | (read8(pc+2) & 0xff);pc+=3;
	...
	uint8_t *handle_jsraa(uint8_t op,uint8_t *pc)
	{
		int imm=(read8(pc))&255;pc++;
	...
	uint8_t *handle_eepmov(uint8_t op,uint8_t *pc)
	{
	...
	//////// NOP
	uint8_t *handle_nop(uint8_t op,uint8_t *pc)
	{
		int b=read8(pc);pc++;
	...
	uint8_t *handle_sleep(uint8_t op,uint8_t b,uint8_t *pc)
	{
	...
	//////// END OF OPS
	void dasm(uint8_t *pc,const char *op,const char *params,...)
	{
	...
	int8_t add8(int value1,int value2,int c = 0,bool keepZ=false)
	{
		value1 = se8(value1);
		value2 = se8(value2);
		c&=1;
	...
	int16_t add16(int value1,int value2)
	{
		value1 = se16(value1);
		value2 = se16(value2);
	...
	int32_t add32(int value1,int value2,bool noccr=false)
	{
		int res=value1+value2;
		if (noccr) return res;
	...
	int8_t sub8(int value1,int value2, int c = 0,bool keepZ=false)
	{
		value1 = se8(value1);
		value2 = se8(value2);
		c&=1;
	...
	int16_t sub16(int value1,int value2)
	{
		value1 = se16(value1);
		value2 = se16(value2);
		int res=value1-value2;
	...
	int32_t sub32(int value1,int value2,bool noccr=false)
	{
		int res=value1-value2;
		if (noccr) return res;
		ccrflags_set(ccr_z,res ? 0 : 1);
